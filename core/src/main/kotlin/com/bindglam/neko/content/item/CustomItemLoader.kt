...
class CustomItemLoader : ContentLoader {
    override fun load(key: Key, config: ConfigurationSection){
        BuiltInRegistries.ITEMS.register(key, CustomItem(NamespacedKey(key.namespace(), key.value()), CUSTOM_ITEM_PROPERTIES_CONFIGURABLE.load(config.getConfigurationSection("properties.item"))!!))
    }
